...
- `Tabulacion_generada.xlsx`: Excel construido por código. Por variable: hoja base (datos, estadísticos, frecuencias y porcentajes), `Ítems <Variable>` (tabla, gráfico, Tabla/Figura, fuente y narrativa por ítem), y `Dimensiones <Variable>` (tabla ancha Suma/Nivel/Código por dimensión y consolidado referenciando la hoja base, fichas de baremo, tabla baremada por nivel con gráfico, narrativa y bloque consolidado — la escala Likert es de los ítems; las dimensiones se miden por niveles). Más `Relaciones` (normalidad calculada sobre V1 total, V2 total y dimensiones de V1; correlaciones Pearson o Rho de Spearman según los Sig.: general V1-V2 y cada dimensión de V1 contra V2), `Correlación` e `Información` (con 2 variables).
