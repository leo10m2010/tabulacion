...
- **Por variable**: hoja base (encabezados combinados de dimensiones/indicadores, códigos `P1..Pn` con numeración continua entre variables, base de datos con columnas Total y Valoración por encuestado, estadísticos por ítem con `SUM/MODE.SNGL/AVERAGE/MEDIAN/STDEV.S/CV`, frecuencias y porcentajes por escala); hoja "Ítems" (tabla Frec/% por ítem con Tabla N, gráfico, Figura N, Fuente/Elaboración e **interpretación narrativa automática**); hoja "Dimensiones" (ficha de baremo, tabla de niveles, base por dimensión con Suma Total y Valoración automática, tabla baremada Calificación/Desde/Hasta/f/% con gráfico y narrativa, más un bloque consolidado de la variable); y hoja "Conteo" (respuestas agregadas por dimensión con gráfico y narrativa).
- **Globales**: hoja "Relaciones" (tabla de sumas por encuestado, pruebas de normalidad KS/Shapiro-Wilk para completar desde SPSS, y correlaciones de Pearson con `CORREL` + Sig. bilateral por pares dimensión/variable), hoja "Correlación" (r vivo, r², interpretación y criterio) y hoja "Información". Las hojas de presentación llevan marco verde y rótulos Tabla/Figura como el formato de tesis.
