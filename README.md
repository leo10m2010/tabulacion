...
- **Backend / API**: `node_app/server.js` (Node 18+, sin frameworks). Autenticación con tokens firmados, usuarios con roles y suscripción, generación vía `POST /generate`. Tabulación va por suscripción (días); **TesisHub Forms va por usos** (1 uso = 1 corrida de llenado, recargables por el admin desde el dashboard).
...
- **Control opcional de correlación** (`controlCorrelacion` + `nivelCorrelacion`): activado, eliges el nivel (muy alta ±0.90-1.00, alta ±0.70-0.89, moderada ±0.40-0.69, baja ±0.20-0.39, muy baja ±0.01-0.19 o nula) y el generador ajusta la simulación hasta caer en el rango — el signo lo da la relación directa/inversa ya elegida; desactivado, la correlación es el resultado natural. Verificación con Spearman (default para Likert) o Pearson; el resultado se muestra en la web y en la hoja "Información". Solo para datos simulados.
